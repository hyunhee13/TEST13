...
<script type="text/javascript">
	...
	function sendIt(){
		...
		f.action = "<%=cp%>/notiCreated_ok.action?pageNum=${pageNum}";
		f.submit();
			...
			<div class="cs_content">
						...
						<tr>
							<td bgcolor="#F5F5F5" style="border:1px solid #dbdbdb; border-bottom:0px; ">
								<table cellpadding="0" cellspacing="0" width="100%">
									<tr>
										<td style="padding:10px 20px 10px 20px;width:600px;"><b><font color=#767676; style="font-size: 15px;">${dto.notiSubject }</font></b></td>
				...
				<c:if test="${sessionScope.adminId=='admin' }">
					<div style="margin: 15px 0px 5px 260px;">
						...
						onclick="javascript:location.href='<%=cp%>/notiUpdated.action?notiNum=${dto.notiNum}&pageNum=${pageNum}';"/>
						<input type="reset" value=" 삭제하기" class="btn2" 
						onclick="javascript:location.href='<%=cp%>/notiDeleted.action?notiNum=${dto.notiNum}&pageNum=${pageNum}';"/>
						<input type="button" value=" 리스트" class="btn2" 
						onclick="javascript:location.href='<%=cp%>/notiList.action?${params}';"/>	
					</div>
				</c:if>
